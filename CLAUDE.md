...
`src/lib/remark-hide-sections.mjs` is a custom remark plugin (registered **first** in `markdown.remarkPlugins`, before `remarkWikilinks`) that hides a section from the published site. **Authoring rule:** prefix any section heading with `!` (e.g. `#### !La Sangre X`) and the build strips that heading plus everything under it — up to the next heading of equal-or-higher level, so a marked parent heading also removes its nested subsections. The `!` stays in the Brain markdown (visible/editable in Obsidian); only the site drops it. Detection is a leading `!` (U+0021) in the heading text; the Spanish `¡` (U+00A1) does not match. Because it runs at the remark stage, removed headings also disappear from `render().headings`. Applies to all collections (single global registration).
